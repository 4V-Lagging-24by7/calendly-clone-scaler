...
import { Clock, Link2, MoreVertical, Plus, Search, ExternalLink, Copy, Pencil, Trash2 } from "lucide-react";
    ...
    <div className="max-w-4xl">
      ...
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
          Scheduling
          <span className="text-gray-400 text-base font-normal cursor-help" title="Manage your event types">⓪</span>
        </h1>
        <CreateEventDropdown
          onSelectOneOnOne={() => { setEditing(null); setModalOpen(true); }}
        />
      </div>
      ...
      <div className="flex items-center justify-between py-3 px-1 mb-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-gray-800">Charvi Singh</span>
        </div>
        <button className="flex items-center gap-1.5 text-sm text-[#006BFF] hover:underline font-medium">
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-white rounded-lg border border-gray-200">
          ...
          <p className="text-gray-500 text-sm">
            {search ? "No event types match your search." : "No event types yet. Create one to get started."}
          </p>
          {!search && (
            <Button
              className="mt-4"
        ...
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden divide-y divide-gray-100">
          {filtered.map((et) => (
            <div key={et.id} className="flex items-center px-5 py-4 hover:bg-gray-50 group relative">
              {/* Color accent bar */}
              ...
              <div className="flex-1 min-w-0 pl-3">
                <div className="flex items-center gap-2 mb-0.5">
                  ...
                  {!et.isActive && (
                    <span className="text-xs text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full">Off</span>
                  )}
                </div>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl p-6 w-full max-w-sm shadow-xl">
            <h3 className="text-lg font-semibold text-gray-900 mb-2">Delete event type?</h3>
            <p className="text-sm text-gray-500 mb-5">
              This will permanently delete this event type and all associated bookings. This cannot be undone.
            </p>
